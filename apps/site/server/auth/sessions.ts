...
import { refreshHashSecret } from '../secrets';
...
export async function getAccessSession(headers: ReadonlyHeaders, sub: string, fam: string) {
  ...
  const tokenWhere = { userId: BigInt(sub), tokenFamily: fam, expiredAt: null };
...
export async function createSession(headers: Headers, sub: string) {
  ...
  const [accessToken, refreshToken] = await Promise.all([
    createJwtToken(sub, fam, TokenType.Access, isAdmin ? 'admin' : 'user'),
    createJwtToken(sub, fam, TokenType.Refresh, isAdmin ? 'admin' : 'user'),
  ]);
