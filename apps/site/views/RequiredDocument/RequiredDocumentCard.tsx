...
export default function RequiredDocumentCard({ requiredDocument }: RequiredDocumentCardProps) {
  ...
  const { id, name, description, isRequired, teamTypes } = requiredDocument;
  ...
  return (
    <div className="flex flex-col gap-2 px-4 py-2 border-2 border-color-1 rounded-md">
      ...
      <div className="flex flex-wrap gap-4">
        <button
          ...
          onClick={() =>
            openModal({ node: <RequiredDocumentModal teamTypes={teamTypes} id={id} defaultValues={defaultValues} /> })
          }
        >
          Modifier
        </button>
