...
export type BankPreviewProps = {
  iban: string;
  holderName: string;
  bicSwift: string;
  bankLocation?: LegalUnitLocationMinimalInfo | null;
};
export default function BankPreview({ iban, holderName, bicSwift, bankLocation }: BankPreviewProps) {
  ...
  if (iban.startsWith('FR')) {
    ...
    if (cleanIban.length > 12) parts.accountNumber[0] = cleanIban.slice(14, 25);
  ...
  return (
    <div className="lg:w-fit h-fit flex flex-col gap-10 lg:rounded-2xl bg-2 pl-6 md:pl-12 pr-6 md:pr-20 py-8">
      ...
      <GroupItem
        ...
        groupClassName={clsx('font-medium', bankLocation ? 'text-0' : 'opacity-50')}
      >
        {bankLocation
