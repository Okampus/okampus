...
export default function BankInfoPreview({ iban, holderName, bicSwift, bankInfoLocation }: BankInfoPreviewProps) {
  ...
  if (iban.startsWith('FR')) {
    ...
    if (cleanIban.length > 4) parts.bankInfo[0] = cleanIban.slice(4, 9);
  ...
  return (
    <div className="lg:w-fit h-fit flex flex-col gap-10 lg:rounded-2xl bg-2 pl-6 md:pl-12 pr-6 md:pr-20 py-8">
      <GroupItem
        heading={t('bankInfo.holderName')}
        groupClassName={clsx('font-medium', holderName ? 'text-0' : 'opacity-50')}
      >
        {holderName || 'XX XXXXXXXXXX XXXXX'}
      </GroupItem>
      <GroupItem heading={t('bankInfo.iban')} groupClassName={clsx('font-medium', iban ? 'text-0' : 'opacity-50')}>
