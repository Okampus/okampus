...
import useTheme from '../../hooks/useDarkSide';
...
const schema = z.object({
  username: z.string().min(1, { message: "Rentre ton nom d'utilisateur pour te connecter." }),
  password: z.string().min(1, { message: "N'oublie pas d'entrer ton mot de passe." }),
});
...
function WelcomePage() {
  ...
  // useEffect(() => {
  //   emitter.emit('finishLoading');
  //   // eslint-disable-next-line react-hooks/exhaustive-deps
  // }, []);

  const { setIsLoading, user, setUser } = useContext(NavigationContext);
  ...
  return (
    <div className="flex w-full h-full overflow-hidden">
      <div className="relative bg-1 w-full h-full flex flex-col justify-center items-center overflow-scroll app-scrollbar">
        <div className="w-96 flex flex-col">
          {/* Logo */}
          <div className="h-14 overflow-hidden relative">
            <div className="text-3xl flex gap-4 items-center absolute bottom-4">
              <div className="bg-gray-50 border bc-2 flex items-center justify-center rounded-lg w-10 h-10">
          ...
          <div className="flex flex-col gap-6 min-h-max shrink-0 h-max py-8">
            ...
            <div>
              ...
              <h3 className="text-2 text-sm sm-max:text-xs flex gap-1.5 items-start">
                Connecte-toi sur le portail de ton école
            ...
            <div className="flex items-center gap-1.5 text-xs text-1 before:h-[1px] before:flex-1 before:bg-gray-300 after:h-[1px] after:flex-1 after:bg-gray-300">
              Ou connecte-toi via ton compte Okampus
            </div>
            {/* Login form */}
              ...
              <div className="flex flex-col gap-4 mb-6">
                ...
                <div>
                  ...
                  {(error || errors.password?.message) && (
                    <p className="pt-1 text-red-400 text-sm">{error?.toString?.() ?? errors.password?.message}</p>
                  )}
                </div>
              </div>
