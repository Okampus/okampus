...
import { capitalize, formatDateDayOfWeek, formatDateDayOfWeekNoHour } from '@okampus/shared/utils';
...
import type { EventRoleBaseInfo, EventDetailsInfo } from '@okampus/shared/graphql';
...
export function EventViewWrapper({ event }: { event: EventDetailsInfo }) {
  const { showSidePanel, hideSidePanel, tenant } = useContext(NavigationContext);
      ...
      element: () => (
        <div className="flex flex-col gap-10">
          <div className="lg:mt-[var(--padding-vertical-view)] lg:px-16">
            <BannerImage src={event.fileUpload?.url} name={event.contentMaster?.name} />
          </div>
          <div className="flex flex-col gap-4 px-content">
            <div className="flex justify-between items-center">
              <div>
                ...
                <div className="text-5xl text-0 font-bold">{event.contentMaster?.name}</div>
              </div>
              <ActionButton
                action={{
                  linkOrActionOrMenu: () =>
            ...
            <div className="mt-6 text-2 font-medium leading-7">{event.contentMaster?.content.text}</div>
          </div>
        </div>
      ),
    },
                    ...
                    {roles.map((role) => (
                      <div className="flex p-3 items-center gap-6 text-0 bg-1-hover rounded-xl" key={role.id as string}>
                        ...
                        <div className="flex-1 w-full text-lg font-bold">
                          {role.event.contentMaster?.name && capitalize(role.event.contentMaster.name)}
                        </div>
  ...
  return <TabsTopbarView menus={menus} basePath={EVENT_ROUTE(event.contentMaster?.slug)} />;
}
