...
export class TeamEventRegistrationsService {
  ...
  public async findOne(user: User, id: number): Promise<TeamEventRegistration> {
    const registration = await this.teamEventRegistrationRepository.findOneOrFail({ id }, { populate: ['user', 'event', 'event.team'] });
  ...
  public async remove(user: User, id: number): Promise<void> {
    ...
    const event = await this.teamEventRepository.findOneOrFail({ id: registration.event.id }, { populate: ['team', 'team.members'] });
