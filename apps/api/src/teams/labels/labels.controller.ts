...
export class LabelsController {
  ...
  @Get(':id')
  @CheckPolicies(ability => ability.can(Action.Read, TeamLabel))
  public async findOne(@Param('id', ParseIntPipe) id: number): Promise<TeamLabel> {
    return await this.labelsService.findOne(id);
  }

  @Patch(':id')
  @CheckPolicies(ability => ability.can(Action.Update, TeamLabel))
  public async update(@Param('id', ParseIntPipe) id: number, @Body() updateLabelDto: UpdateLabelDto): Promise<TeamLabel> {
