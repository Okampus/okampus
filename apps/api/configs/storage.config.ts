...
const awsLogRegex =
  /\[AWS (?<service>\w+) (?<statusCode>[\d\w]+) (?<time>[\d.]+)s (?<retries>\d+) retries\] (?<method>\w+)\(.*\)/isu;
  ...
  config: {
    ...
    logger: {
      log: (message: string) => {
        ...
        storageLogger.log(
          `${awsLog?.method}: ${awsLog?.time}s - ${
            awsLog?.statusCode === 'undefined' ? '<Timed out>' : awsLog?.statusCode
          } [${awsLog?.retries} retries]`
        );
      },
    },
  },
