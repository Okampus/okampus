...
export const defaultStarterKit = StarterKit.configure({
    heading: {
        levels: [1, 2, 3]
    }
})

export const defaultLink = Link.configure({
    HTMLAttributes: {
        class: 'link-blue',
    },
})
...
export function extractTextFromHTML (html, space) {
    ...
    if (space) { // add a space after each span element
    ...
    return [span.textContent || span.innerText].toString() // get text from span and convert to string
        .replace(/ +/g, ' ') // remove multiple spaces
}

export function extractTextFromTipTapJSON (json=defaultTipTapText, extensions=defaultTipTapExtensions, space=true) {
