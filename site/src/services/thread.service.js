...
class ThreadService {
    async fetchThread (id) {
        const replies = await axios.get(`${API_URL}/posts/${id}/replies`, { withCredentials: true }).then(res => res.data.items);

        const rawResponses = await Promise.all([
            axios.get(`${API_URL}/posts/${id}`, { withCredentials: true }),
            axios.get(`${API_URL}/posts/${id}/comments`, { withCredentials: true }),
            ...replies.map(reply => axios.get(`${API_URL}/posts/replies/${reply.replyId}/comments`, { withCredentials: true })),
        ]);

        const rawVoteResponses = await Promise.all([
            axios.get(`${API_URL}/posts/${id}/vote`, { withCredentials: true }),
            ...replies.map(reply => axios.get(`${API_URL}/posts/replies/${reply.replyId}/vote`, { withCredentials: true }))
        ])

        const rawFavoriteResponses = await Promise.all([
            axios.get(`${API_URL}/favorites/posts/${id}`, { withCredentials: true }).then(() => true).catch(() => false),
            ...replies.map(reply => axios.get(`${API_URL}/favorites/replies/${reply.replyId}`, { withCredentials: true }).then(() => true).catch(() => false))
        ])
        ...
        const rawFavoriteComments = await Promise.all(post.comments.map(
            comment => axios.get(`${API_URL}/favorites/comments/${comment.commentId}`, { withCredentials: true }).then(() => true).catch(() => false)
        ))
        ...
        for (const reply of replies) {
            ...
            const rawFavoriteComments = await Promise.all(reply.comments.map(
                comment => axios.get(`${API_URL}/favorites/comments/${comment.commentId}`, { withCredentials: true }).then(() => true).catch(() => false)
            ))
    ...
    addReply(postId, body) {
        return axios.post(`${API_URL}/posts/${postId}/replies`, { body }, { withCredentials: true }).then(
            res => res.data
        )
    }

    updateReply ({ replyId, body }) {
        return axios.patch(`${API_URL}/posts/replies/${replyId}`, { body }, { withCredentials: true }).then(
            res => res.data
        )
    }

    addPostComment(postId, body) {
        return axios.post(`${API_URL}/posts/${postId}/comments`, { body }, { withCredentials: true }).then(
            res => res.data
        )
    }

    addReplyComment(replyId, body) {
        return axios.post(`${API_URL}/posts/replies/${replyId}/comments`, { body }, { withCredentials: true }).then(
            res => res.data
        )
    }

    updateComment(commentId, body) {
        return axios.patch(`${API_URL}/posts/replies/comments/${commentId}`, { body }, { withCredentials: true }).then(
            res => res.data
        )
    }
    ...
    favoriteComment(commentId) {
        return axios.post(`${API_URL}/favorites/comments/${commentId}`, {}, { withCredentials: true }).then(
            res => res.data
        )
    }
}
