...
import type { Event } from '@okampus/api/dal';
import type {
  EventInsertInput,
  EventOnConflict,
  EventBoolExp,
...
export class EventsService extends RequestContext {
  ...
  async checkPermsDelete(event: Event) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, event))) return true;
  ...
  async checkPermsUpdate(props: EventSetInput, event: Event) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, event))) return true;
