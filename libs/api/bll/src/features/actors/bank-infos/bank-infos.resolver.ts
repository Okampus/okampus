...
export class BankInfosQueryResolver {
  ...
  async deleteBankInfoByPk(@Info() info: GraphQLResolveInfo) {
    const { id } = getGraphQLArgs<DeleteByPkBankInfoArgsType>(
      info.parentType.getFields()[info.fieldName],
      info.fieldNodes[0],
      info.variableValues
    );
    return await this.bankInfosService.deleteBankInfoByPk(getSelectionSet(info), id);
  }
}
