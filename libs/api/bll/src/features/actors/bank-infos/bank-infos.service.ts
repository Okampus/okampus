...
export class BankInfosService extends RequestContext {
  ...
  async checkPermsDelete(bankInfo: BankInfo) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, bankInfo))) return true;
  ...
  async checkPermsUpdate(props: BankInfoSetInput, bankInfo: BankInfo) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, bankInfo))) return true;
