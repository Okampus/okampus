...
import type { Form } from '@okampus/api/dal';
import type {
  FormInsertInput,
  FormOnConflict,
  FormBoolExp,
...
export class FormsService extends RequestContext {
  ...
  async checkPermsDelete(form: Form) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, form))) return true;
  ...
  async checkPermsUpdate(props: FormSetInput, form: Form) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, form))) return true;
