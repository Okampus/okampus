...
export class ExpenseItemsService extends RequestContext {
  ...
  async updateExpenseItemMany(
    ...
    await Promise.all(updates.map(async (update) => {
      ...
      if (!canUpdate) throw new ForbiddenException(`You are not allowed to update ExpenseItem (${update.where.id._eq}).`);
    ...
    await Promise.all(expenseItems.map(async (expenseItem) => {
  ...
  async deleteExpenseItem(
    ...
    const data = await this.hasuraService.update('updateExpenseItem', selectionSet, where, { deletedAt: new Date().toISOString() });
  ...
  async deleteExpenseItemByPk(
    ...
    const data = await this.hasuraService.updateByPk('updateExpenseItemByPk', selectionSet, { id }, {
  ...
  async aggregateExpenseItem(
    ...
    // Custom logic
    const data = await this.hasuraService.aggregate('expenseItemAggregate', selectionSet, where, orderBy, distinctOn, limit, offset);
