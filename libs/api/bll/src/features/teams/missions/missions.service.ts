...
import { MissionRepository, Mission } from '@okampus/api/dal';
...
export class MissionsService extends RequestContext {
  ...
  checkPermsDelete(mission: Mission) {
    ...
    if (
      this.requester()
        ...
        .some(
          (role) =>
            role.permissions.includes(AdminPermissions.DeleteTenantEntities) && role.tenant?.id === mission.tenant?.id,
        )
    )
      return true;
  ...
  checkPermsUpdate(props: MissionSetInput, mission: Mission) {
    ...
    if (
      this.requester()
        ...
        .some(
          (role) =>
            role.permissions.includes(AdminPermissions.ManageTenantEntities) && role.tenant?.id === mission.tenant?.id,
        )
    )
      return true;
  ...
  async findMissionByPk(selectionSet: string[], id: string) {
    // Custom logic
    const data = await this.hasuraService.findByPk('missionByPk', selectionSet, { id });
    return data.missionByPk;
  }

  async insertMission(selectionSet: string[], objects: Array<MissionInsertInput>, onConflict?: MissionOnConflict) {
  ...
  async updateMissionMany(selectionSet: string[], updates: Array<MissionUpdates>) {
  ...
  async deleteMission(selectionSet: string[], where: MissionBoolExp) {
