...
export class BankAccountsService extends RequestContext {
  ...
  async checkPermsDelete(bankAccount: BankAccount) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, bankAccount))) return true;
  ...
  async checkPermsUpdate(props: BankAccountSetInput, bankAccount: BankAccount) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, bankAccount))) return true;
  ...
  async updateBankAccountMany(
    ...
    await Promise.all(updates.map(async (update) => {
      ...
      if (!canUpdate) throw new ForbiddenException(`You are not allowed to update BankAccount (${update.where.id._eq}).`);
  ...
  async deleteBankAccount(
    ...
    const data = await this.hasuraService.update('updateBankAccount', selectionSet, where, { deletedAt: new Date().toISOString() });
  ...
  async deleteBankAccountByPk(
    ...
    const data = await this.hasuraService.updateByPk('updateBankAccountByPk', selectionSet, { id }, {
  ...
  async aggregateBankAccount(
    ...
    // Custom logic
    const data = await this.hasuraService.aggregate('bankAccountAggregate', selectionSet, where, orderBy, distinctOn, limit, offset);
