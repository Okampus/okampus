...
export class TeamMembersService extends RequestContext {
  ...
  checkPermsDelete(teamMember: TeamMember) {
    ...
    if (this.requester().adminRoles.getItems().some((role) => role.permissions.includes(AdminPermissions.DeleteTenantEntities) && role.tenant?.id === teamMember.tenant?.id)) 
      return true;
  ...
  checkPermsUpdate(props: TeamMemberSetInput, teamMember: TeamMember) {
    ...
    if (this.requester().adminRoles.getItems().some((role) => role.permissions.includes(AdminPermissions.ManageTenantEntities) && role.tenant?.id === teamMember.tenant?.id)) 
      return true;
