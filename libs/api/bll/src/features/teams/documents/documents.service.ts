...
export class DocumentsService extends RequestContext {
  ...
  async checkPermsDelete(document: Document) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, document))) return true;
  ...
  async checkPermsUpdate(props: DocumentSetInput, document: Document) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, document))) return true;
