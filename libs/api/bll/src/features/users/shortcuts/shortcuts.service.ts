...
export class ShortcutsService extends RequestContext {
  ...
  async checkPermsDelete(shortcut: Shortcut) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, shortcut))) return true;
  ...
  async checkPermsUpdate(props: ShortcutSetInput, shortcut: Shortcut) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, shortcut))) return true;
  ...
  async updateShortcutMany(
    ...
    await Promise.all(updates.map(async (update) => {
      ...
      if (!canUpdate) throw new ForbiddenException(`You are not allowed to update Shortcut (${update.where.id._eq}).`);
  ...
  async deleteShortcut(
    ...
    const data = await this.hasuraService.update('updateShortcut', selectionSet, where, { deletedAt: new Date().toISOString() });
  ...
  async deleteShortcutByPk(
    ...
    const data = await this.hasuraService.updateByPk('updateShortcutByPk', selectionSet, { id }, {
  ...
  async aggregateShortcut(
    ...
    // Custom logic
    const data = await this.hasuraService.aggregate('shortcutAggregate', selectionSet, where, orderBy, distinctOn, limit, offset);
