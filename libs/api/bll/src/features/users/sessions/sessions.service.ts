...
export class SessionsService extends RequestContext {
  ...
  async checkPermsDelete(session: Session) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminDelete(adminRole, session))) return true;
  ...
  async checkPermsUpdate(props: SessionSetInput, session: Session) {
    ...
    const requesterRoles = this.requester().adminRoles.getItems();
    if (requesterRoles.some((adminRole) => canAdminManage(adminRole, session))) return true;
  ...
  async deleteSession(
    ...
    const data = await this.hasuraService.update('updateSession', selectionSet, where, { deletedAt: new Date().toISOString() });
  ...
  async deleteSessionByPk(
    ...
    const data = await this.hasuraService.updateByPk('updateSessionByPk', selectionSet, { id }, {
  ...
  async aggregateSession(
    ...
    // Custom logic
    const data = await this.hasuraService.aggregate('sessionAggregate', selectionSet, where, orderBy, distinctOn, limit, offset);
