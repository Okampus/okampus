...
export class BotsService extends RequestContext {
  ...
  checkPermsDelete(bot: Bot) {
    ...
    if (this.requester().adminRoles.getItems().some((role) => role.permissions.includes(AdminPermissions.DeleteTenantEntities) && role.tenant?.id === bot.tenant?.id)) 
      return true;
  ...
  checkPermsUpdate(props: BotSetInput, bot: Bot) {
    ...
    if (this.requester().adminRoles.getItems().some((role) => role.permissions.includes(AdminPermissions.ManageTenantEntities) && role.tenant?.id === bot.tenant?.id)) 
      return true;
  ...
  async deleteBot(
    ...
    const data = await this.hasuraService.update('updateBot', selectionSet, where, { deletedAt: new Date().toISOString() });
  ...
  async deleteBotByPk(
    ...
    const data = await this.hasuraService.updateByPk('updateBotByPk', selectionSet, { id }, {
  ...
  async aggregateBot(
    ...
    // Custom logic
    const data = await this.hasuraService.aggregate('botAggregate', selectionSet, where, orderBy, distinctOn, limit, offset);
