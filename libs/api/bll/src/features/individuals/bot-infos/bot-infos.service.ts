...
export class BotInfosService extends RequestContext {
  ...
  async insertBotInfo(
    ...
    const botInfo = await this.botInfoRepository.findOneOrFail(data.insertBotInfo[0].id);
    await this.logsService.createLog(botInfo);
  ...
  async updateBotInfoByPk(
    ...
    await this.logsService.updateLog(botInfo, _set);
