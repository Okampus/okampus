...
import { formatDateRange, fuseClasses } from '@okampus/shared/utils';
...
export function EventCard({ event, classes, onClick }: EventCardProps) {
  ...
  return (
    <div
      className={fuseClasses('flex flex-col gap-2 border bc-2 rounded-lg bg-2 py-2 px-4', classes)}
      onClick={onClick}
    >
      ...
      <div className="flex gap-6">
        <div className="flex gap-2 items-center text-2">
          <Avatar
            ...
            name={event.rootContent?.org?.actor?.name}
          />
          <div>{event.rootContent?.org?.actor?.name ?? '?'}</div>
        </div>
        <div className="flex gap-2 items-center text-2">
