...
const formKitListValue = z.union([
  z.string(),
  z.record(z.any()),
  z.array(z.string().or(z.number()).or(z.record(z.any()))),
  z.number(),
]);

const formKitListStatement = z.union([
  z.tuple([z.any(), z.number().or(z.string()), formKitListValue]),
  z.tuple([z.any(), formKitListValue]),
]);

const formKitSchemaMeta: z.ZodType<unknown> = z.lazy(() =>
  z.record(z.string().or(z.number()).or(z.boolean()).or(z.null()).or(z.undefined()).or(formKitSchemaMeta))
);
...
const formKitSchemaAttributes: z.ZodType<unknown> = z.lazy(() =>
  z.union([formKitSchemaAttributesCondition, z.record(formKitAttributeValue), z.null()])
);
...
const formKitSchemaComponent = z
  .object({
    $cmp: z.string(),
